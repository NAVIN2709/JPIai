...
import { updateGeneration, getUser } from "../../firebase/functions/UserFunctions.js";
      ...
      try {
        ...
        if (statusData.status === "done" && statusData.url) {
          ...
          setMessages((prev) =>
            prev.map((m) =>
              m.id === placeholderId
                ? { ...m, status: "failed", error: statusData.error || "Processing failed" }
                : m
            )
          );
          setLoading(false);

        } else if (["queued", "processing", "rendering"].includes(statusData.status)) {
      ...
      {showFCMModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-6">
          <div className="bg-white text-black p-6 rounded-lg max-w-sm w-full shadow-lg">
            ...
            <p className="mb-4">
              Meanwhile, try solving the question yourself — but stay updated while
              your result processes!
            </p>
          ...
          >
            <div className="max-w-xs md:max-w-md relative">
              ...
              {msg.type === "user" && msg.file && (
                <img src={msg.file} alt="User" className="rounded-lg shadow-md" />
              )}

              {/* Response Placeholder */}
              {msg.type === "response" && msg.status === "loading" && <SkeletonVideo />}
